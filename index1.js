...
for(var i = 0 ; i < events.length ; i++)
{
  for(var j = 0 ; j < bars.length ; j++)
  {
    if(events[i].barId == bars[j].id)
    {
      ...
      {
        price_c = price_c + price_c*0.1;

      }
      if(events[i].persons > 20)
      {
        price_c = price_c + price_c*0.2;
      }
      if(events[i].persons > 30)
      {
        price_c = price_c + price_c*0.3;

      }
      if (events[i].options.deductibleReduction == true)
      {
        price_c = price_c + events[i].persons;
      }
      
      events[i].price = price_c;

      var commission_p = 0.3 * price_c;
      var insurance_p = 0.5 * commission_p;

      events[i].commission.insurance = insurance_p;
      events[i].commission.treasury = events[i].persons;

      var privateaser_p = commission_p - (events[i].persons + insurance_p);
      if (privateaser_p<=0)
      {
        privateaser_p=0;
      }
      events[i].commission.privateaser = privateaser_p;
    }
  }
}
